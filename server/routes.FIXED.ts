/**
 * FIXED VERSION - API routes with security and performance improvements
 * 
 * Changes:
 * 1. Added rate limiting to all endpoints
 * 2. Fixed XSS vulnerabilities in email templates
 * 3. Added caching for crypto rates
 * 4. Fixed file size calculation
 * 5. Added timeout for external API calls
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // FIXED: Added rate limiting, input sanitization, and XSS protection
  ...
  // FIXED: Added rate limiting for promo validation
  app.post("/api/promo/validate", rateLimiters.promo, async (req, res) => {
    try {
      const { code } = req.body;
  ...
  // FIXED: Added rate limiting and XSS protection
