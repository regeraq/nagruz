...
> ⚠️ **Функция пока не реализована.** Ранее в этом разделе была описана
> вкладка «Аналитика» и эндпоинт `/api/admin/audit-logs` — ни того, ни другого
> в приложении нет. Описание убрано, чтобы на него не рассчитывали при
> разборе инцидентов.

### Текущее состояние

Таблица `admin_audit_log` заведена в схеме БД (`shared/schema.ts`) и создаётся
миграцией, но **серверный код в неё ничего не пишет и не читает**.
Она зарезервирована под будущую реализацию.

Действия администраторов сейчас попадают только в логи приложения:

```bash
pm2 logs loaddevice --lines 200
```

В логах пишется `userId` администратора, а не e-mail — это сделано намеренно,
чтобы не дублировать персональные данные в журналах (152-ФЗ, минимизация).
Сопоставить `userId` с человеком можно в разделе «Пользователи».

### Чего не хватает для полноценного журнала

Записи в `admin_audit_log` при изменении товаров, ролей, статусов заказов и
промокодов; эндпоинт для чтения с фильтрами; интерфейс в админке. Логи PM2
ротируются и не предназначены для долгого хранения, поэтому полагаться на них
как на юридически значимый журнал нельзя.
