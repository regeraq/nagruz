...
### Строка подключения
```
postgresql://loaddevice_user:ПАРОЛЬ@localhost:5432/loaddevice_db
```

**Формат:** `postgresql://[пользователь]:[пароль]@[хост]:[порт]/[база_данных]`

Пароль генерируйте случайным (`openssl rand -base64 24`) и храните только
в `.env` с правами `600`. Простые пароли вида `loaddevice123` приложение
в production не примет — проверка при старте (`server/preflight.ts`)
остановит запуск.
...
Лежат в `migrations/`, имена генерирует Drizzle Kit автоматически.
Базовая (первая) миграция — `migrations/0000_silly_firedrake.sql`.
Служебные файлы `migrations/meta/` (снапшот схемы и журнал применённых
версий) **обязательно** коммитятся вместе с SQL — без них Drizzle не сможет
сгенерировать следующую миграцию корректно.

### Применение миграций

**Штатный способ (и локально, и на сервере):**
```bash
npm run db:migrate
```

Команда применяет все ещё не применённые SQL-файлы из `migrations/` и
записывает применённые версии в служебную таблицу, поэтому повторный
запуск безопасен.

**Изменение схемы** — правим `shared/schema.ts`, затем **локально**:
```bash
npm run db:generate     # создаст новый SQL-файл в migrations/
git add migrations/ shared/schema.ts && git commit && git push
```

На сервере миграция применится при обновлении (`update-project.sh`).

> ⚠️ **`npm run db:push` — только для локальной разработки.** Команда
> сравнивает схему с БД напрямую и **интерактивно** спрашивает про
> переименование и усечение таблиц. В неинтерактивной сессии (SSH-команда,
> CI/CD) она либо зависает, либо получает неверный ответ и может
> переименовать или обрезать таблицу с живыми данными. На сервере всегда
> используйте `db:migrate`.

**Важно:** права на схему `public` должны быть выданы пользователю
`loaddevice_user` от имени `postgres` — иначе приложение не сможет
создавать таблицы (актуально для PostgreSQL 15+).
...
2. Перейдите на вкладку **«БД»**
3. Нажмите «Экспортировать базу данных»
4. Скачается файл **`database-backup-YYYY-MM-DD.sql`**

> Экспорт доступен только роли **superadmin**. Файл содержит персональные
> данные пользователей — храните его как секрет и удаляйте, когда он больше
> не нужен.

Это удобный способ снять срез вручную, но **не замена регулярным бэкапам** —
для них используйте `scripts/backup.sh` по расписанию (§15 в
