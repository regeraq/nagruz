...
# pipefail: без него падение команды в середине конвейера остаётся незамеченным
# (как в scripts/backup.sh и scripts/restore.sh).
set -eo pipefail
...
# Раньше вывод уходил в /dev/null, а неудачный бэкап был лишь предупреждением —
# то есть миграция могла примениться вообще без свежей копии данных.
if [ -f "scripts/backup.sh" ]; then
    ...
    if bash scripts/backup.sh >"$BACKUP_LOG" 2>&1; then
        ...
        echo "   Лог: $BACKUP_LOG"
        tail -n 20 "$BACKUP_LOG" || true
        echo ""
        echo "   Накатывать миграции без свежей копии БД нельзя: откат кода"
        echo "   не вернёт испорченные данные. Исправьте бэкап и повторите."
        echo "   Осознанно пропустить: SKIP_BACKUP=1 bash update-project.sh"
        [ "${SKIP_BACKUP:-0}" = "1" ] || exit 1
        echo "   ⚠️  SKIP_BACKUP=1 — продолжаем на свой риск"
    fi
    echo ""
fi
